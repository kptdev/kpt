...
func (pn *pkgNode) runValidators(ctx context.Context, hctx *hydrationContext, input []*yaml.RNode) error {
	...
	for i := range pl.Validators {
		...
		validator, err = runtime.NewRunner(ctx, hctx.fileSystem, &function, pn.pkg.UniquePath, hctx.fnResults, opts, hctx.runtime)
		if err != nil {
			hctx.validationSteps = append(hctx.validationSteps, preExecFailureStep(function, err))
			return err
		}
		validatorRunner := validator.(*runtime.FunctionRunner)
...
func fnChain(ctx context.Context, hctx *hydrationContext, pkgPath types.UniquePath, fns []kptfilev1.Function) ([]*runtime.FunctionRunner, int, error) {
