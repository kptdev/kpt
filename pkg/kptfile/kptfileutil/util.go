...
func addNameForMerge(kfs ...*kptfilev1.KptFile) {
	for _, kf := range kfs {
		...
		for i, mutator := range kf.Pipeline.Mutators {
			kf.Pipeline.Mutators[i] = addName(mutator, nameMap)
		}
		for i, validator := range kf.Pipeline.Validators {
			kf.Pipeline.Validators[i] = addName(validator, nameMap)
		}
	}
}

// addName adds name field to the input function if empty
// name is generated by combination of image name(excluding version), input
// config type(configMap or configPath or empty) and nth occurrence of function
func addName(fn kptfilev1.Function, nameMap map[string]int) kptfilev1.Function {
	if fn.Name == "" {
		image := strings.Split(fn.Image, ":")[0]
		image = strings.TrimPrefix(image, "gcr.io/kpt-fn/")
		var configOption string
		if fn.ConfigPath != "" {
			configOption = "configPath"
		} else if fn.ConfigMap != nil {
			configOption = "configMap"
		}
		ind := nameMap[fmt.Sprintf("_kpt-merge_%s_%s", image, configOption)]
		// if function is declared multiple times, ind makes the name unique
		nameMap[fmt.Sprintf("_kpt-merge_%s_%s", image, configOption)] = ind + 1
		fn.Name = fmt.Sprintf("_kpt-merge_%s_%s_%d", image, configOption, ind+1)
	}
	return fn
}
