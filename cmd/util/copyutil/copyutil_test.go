...
// TestDiff_skipKptfileSrc verifies that the KptFile in the source is not
// looked at
func TestDiff_skipKptfileSrc(t *testing.T) {
	s, err := ioutil.TempDir("", "copyutilsrc")
	assert.NoError(t, err)
	d, err := ioutil.TempDir("", "copyutildest")
	assert.NoError(t, err)
...
// TestDiff_skipKptfileSrc verifies that the KptFile in the destination is not
// looked at
func TestDiff_skipKptfileDest(t *testing.T) {
	s, err := ioutil.TempDir("", "copyutilsrc")
	assert.NoError(t, err)
	d, err := ioutil.TempDir("", "copyutildest")
	assert.NoError(t, err)
