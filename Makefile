...
test-docker: build
	KPT_E2E_BIN=$(GOBIN)/kpt PATH=$(GOBIN):$(PATH) go test -cover --tags=docker ./...
...
test-fn-render: build
	KPT_E2E_BIN=$(GOBIN)/kpt PATH=$(GOBIN):$(PATH) go test -v --tags=docker --run=TestFnRender ./e2e/

# target to run e2e tests for "kpt fn eval" command
test-fn-eval: build
	KPT_E2E_BIN=$(GOBIN)/kpt PATH=$(GOBIN):$(PATH) go test -v --tags=docker --run=TestFnEval ./e2e/
