...
func (src *PackageVariantSet) ConvertTo(dstRaw conversion.Hub) error {
	dst := dstRaw.(*v1alpha2.PackageVariantSet)

	return ConvertV1alpha1ToV1alpha2(src, dst)
}

func (dst *PackageVariantSet) ConvertFrom(srcRaw conversion.Hub) error {
	src := srcRaw.(*v1alpha2.PackageVariantSet)
	...
	for _, t := range src.Spec.Targets {
		...
		for _, r := range t.Repositories {
			if len(r.PackageNames) == 0 {
				...
				dst.Spec.Targets = append(dst.Spec.Targets, Target{
					Package: &Package{
						Repo: r.Name,
						Name: pkg,
					},
...
func ConvertV1alpha1ToV1alpha2(src *PackageVariantSet, dst *v1alpha2.PackageVariantSet) error {
	dst.ObjectMeta = src.ObjectMeta
