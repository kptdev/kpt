...
var Filters = map[string]func() Filter{
	"Modifier":   func() Filter { return &Modifier{} },
	"FileSetter": func() Filter { return &FileSetter{} },
}
...
func (f *FileSetter) Filter(input []*yaml.RNode) ([]*yaml.RNode, error) {
	...
	for i := range input {
		...
		if _, found := m.Annotations[PathAnnotation]; !found || f.Override {
			if _, err := input[i].Pipe(yaml.SetAnnotation(PathAnnotation, file)); err != nil {
				return nil, err
			}
		}
		if _, found := m.Annotations[ModeAnnotation]; !found || f.Override {
			if _, err := input[i].Pipe(yaml.SetAnnotation(ModeAnnotation, f.Mode)); err != nil {
				return nil, err
			}
		}
	...
	for i := range resources {
		if err := sortNodes(resources[i]); err != nil {
			return nil, err
		}
		for j := range resources[i] {
			if _, err := resources[i][j].Pipe(
				yaml.SetAnnotation(IndexAnnotation, fmt.Sprintf("%d", j))); err != nil {
				return nil, err
			}
